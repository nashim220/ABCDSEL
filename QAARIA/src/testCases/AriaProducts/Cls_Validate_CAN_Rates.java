/*
 Author     		:	Abhishek
 Class Name			: 	Cls_Validate_CAN_Rates 
 Purpose     		: 	Purpose of this file is :
						1. To Validate CAN Rates.
...
public class Cls_Validate_CAN_Rates extends VerificationMethods
{

WebElement webElement;
public static WebDriver driver;
	
	public static String strTestCaseName = "Cls_Validate_CAN_Rates";
		...
		@Test(dataProvider="objectTestData", description="Validate_CAN_Rates")
	    public void ValidateCANRates(String strAccNum,String strplanName) throws Exception
	    {
	    	
			WebDriverWait webWait;
			webWait = new WebDriverWait(driver,1000);
		    Log.startTestCase("Cls_Validate_CAN_Rates");
		    Log.info("the plan name is "+strplanName);
		    
		    
		   try
			{
			   AriaEOM ObjCANRates = new AriaEOM();
			   
			   //Clicking on link products
			   ObjCANRates.fn_clickProducts(driver, webWait).click();
			   Thread.sleep(500);
			   
			   //Clicking on link plans
			   ObjCANRates.fn_clickProductsPlans(driver, webWait).click();
			   Thread.sleep(15000);
			 ...
			 Thread.sleep(30000);
			 	...
			 	//TODO: Read the CAN Rates  file for data comparison and verification.
				...
				ArrayList<String> arrlistFileContents = ReadFiles.readExcelFileAsArray(fileRates, "CAN_Rates");
			...
			//For fetching the table Xpath of CAN Currency
            Log.info("/-*******************************************************************-/");
            
            Log.info("/-*********Service Names and CAN Rates Retrieved From Application**********-/");
					...
					if(cells.size()	> 1){
						for(WebElement Outercol : cells){
							...
							if(Ratecells.getAttribute("class").equals("ratePerUnit")){
								CANRate = Ratecells.getAttribute("value");
							}
						}
						
					}
					if(ServiceName != "" && CANRate !="")
					{	
						map.put(ServiceName,CANRate);
					}		 
			}
			...
			Log.info("HashMap Compared, CAN Rates match : " + ExcelDatamap.equals(map));
			Log.info("/-*********************************************************-/");
			
			}  
			catch (Exception exception)
			{
				Log.error("There is some error in Validation of CAN Rates");
			}
	    ...
	    public Object[][] data() throws DataDrivenFrameworkException
	    {
	        ...
	        return objUtils.data1("TestCaseDetails", "Validate_CAN_Rates");
	    }
