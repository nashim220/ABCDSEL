/*
 Author     		:	Abhishek
 Class Name			: 	Cls_Validate_EURO_Rates 
 Purpose     		: 	Purpose of this file is :
						1. To Validate EURO Rates.
...
public class Cls_Validate_EURO_Rates extends VerificationMethods
{

WebElement webElement;
public static WebDriver driver;
	
	public static String strTestCaseName = "Cls_Validate_EURO_Rates";
		...
		@Test(dataProvider="objectTestData", description="Validate_EURO_Rates")
	    public void ValidateEURORates(String strAccNum,String strplanName) throws Exception
	    {
	    	
			WebDriverWait webWait;
			webWait = new WebDriverWait(driver,1000);
		    Log.startTestCase("Cls_Validate_EURO_Rates");
		    Log.info("the plan name is "+strplanName);
		    
		    
		   try
			{
			   AriaEOM ObjEURORates = new AriaEOM();
			   
			   //Clicking on link products
			   ObjEURORates.fn_clickProducts(driver, webWait).click();
			   Thread.sleep(500);
			   
			   //Clicking on link plans
			   ObjEURORates.fn_clickProductsPlans(driver, webWait).click();
			   Thread.sleep(15000);
				...
				for (WebElement rowElement: TotalRowCount) 
				{
						...
						for(WebElement colElement:TotalColumnCount)
						{
							...
							{
								colElement.click();
								Log.info("the requested plan number is found : "+strplanName);
							}
			 ...
			 Thread.sleep(30000);
			 	...
			 	//TODO: Read the EURO Rates  file for data comparison and verification.
				...
				ArrayList<String> arrlistFileContents = ReadFiles.readExcelFileAsArray(fileRates, "EURO_Rates");
			...
			//For fetching the table Xpath of EURO Currency
            Log.info("/-*******************************************************************-/");
                       
            Log.info("/-*********Service Name and EURO Rate Retrieved From Application**********-/");
					...
					if(cells.size()	> 1){
						for(WebElement Outercol : cells){
							...
							if(Ratecells.getAttribute("class").equals("ratePerUnit")){
								EURORate = Ratecells.getAttribute("value");
							}
						}
						
					}
					if(ServiceName != "" && EURORate !="")
					{	
						map.put(ServiceName,EURORate);
					}		 
			}
			...
			Log.info("HashMap Compared, EURO Rates match : " + ExcelDatamap.equals(map));
			...
			{
				Log.error("There is some error in Validation of EURO Rates");
			}
	    ...
	    public Object[][] data() throws DataDrivenFrameworkException
	    {
	        ...
	        return objUtils.data1("TestCaseDetails", "Validate_EURO_Rates");
	    }
