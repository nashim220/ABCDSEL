/*
 Author     		:	Abhishek
 Class Name			: 	Cls_Validate_SEK_Rates 
 Purpose     		: 	Purpose of this file is :
						1. To Validate SEK Rates.
...
public static WebDriver driver;
	...
	public static String strTestCaseName = "Cls_Validate_SEK_Rates";
		...
		public void ValidateSEKRates(String strAccNum,String strplanName) throws Exception
	    {
		   ...
		   try
			{
			 ...
			 Thread.sleep(30000);
				...
				ArrayList<String> arrlistFileContents = ReadFiles.readExcelFileAsArray(fileRates, "SEK_Rates");
				...
				for(String key : ExcelDatamap.keySet())
				{
					...
					//System.out.println(key + " : " +ExcelDatamap.get(key));
				}
						
			int Counter=5;
					         
			//For fetching the table Xpath of SEK Currency
            Log.info("/-*******************************************************************-/");
            
            
            Log.info("/-*********Service Name and SEK Rate Retrieved From Application**********-/");
					...
					if(cells.size()	> 1){
						for(WebElement Outercol : cells){
							...
							if(Ratecells.getAttribute("class").equals("ratePerUnit")){
								SEKRate = Ratecells.getAttribute("value");
							}
								
						}
					}
					if(ServiceName != "" && SEKRate !="")
					{	
						map.put(ServiceName,SEKRate);
					}		 
			}
			...
			Log.info("HashMap Compared, SEK Rates match : " + ExcelDatamap.equals(map));
			Log.info("/-*********************************************************-/");
