...
import pageObjects.ObjectsGetAuthenticationKey;

public class Cls_GetAuthenticationKey {
	
	static ObjectsGetAuthenticationKey objTest = new ObjectsGetAuthenticationKey();
	...
	public String fn_GetAuthenticationKey(WebDriver driver, WebDriverWait webWait) throws InterruptedException{
		...
		objTest.fn_ConfigurationLink(driver, webWait).click();
		//click on Integrations link
		objTest.fn_IntegrationsLink(driver, webWait).click();
		//click on web services api link
		objTest.fn_WebServiceAPILink(driver, webWait).click();
		//Read value for Client Number
		String strClientNo = objTest.fn_ClientNumber(driver, webWait).getText();
		String strGenratedClientNo = strClientNo.substring(14, 21);
		//Read value of auth key and store in the variable
		String strAuthenticationKey = objTest.fn_AuthenticationKey(driver, webWait).getText();
				
		String strClientNoAndAuthKey = strGenratedClientNo+"#"+strAuthenticationKey;
