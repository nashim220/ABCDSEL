...
public class UntimedCourseTest 
{
	...
	*/ 
	...
	private void Untimed_TimerNotDisplayed() throws Exception
	{
		...
		System.out.println("Step 1 : Navigate to untimed course State page : " + Constants.ApplicationURL_Snowmobile + Constants.State_Snowmobile_UnTimed);
		Reporter.log("Step 1 : Navigate to untimed course State page : " + Constants.ApplicationURL_Snowmobile + Constants.State_Snowmobile_UnTimed); 
		
			SeleniumFunc.ToGoToUrl(Constants.ApplicationURL_Snowmobile + Constants.State_Snowmobile_UnTimed);
	...
	*/ 
	...
	private void Untimed_AccessTopicRandomly() throws Exception
	{
		...
		Reporter.log("Step 6 : Verifying whether user is navigated to correct Page or not"); 
			...
			System.out.println("Step 7 : Expanding Unit 4, Expanding Topic 4 , clicking on Page 2 link");
			Reporter.log("Step 7 : Expanding Unit 4, Expanding Topic 4 , clicking on Page 2 link"); 
				...
				expectedurl = content.Untimed_GetUnitHrefLink(4, 4, 2)+"/";
				content.Untimed_ClickOnUnitTopicPageLink(4,4,2);
				Thread.sleep(4000);
			...
			Reporter.log("Step 8 : Verifying whether user is navigated to correct Page or not"); 
					...
					if(Actualurl.contains(expectedurl))
					{
						System.out.println("Success !! User is navigated to Unit 4 > Topic 4 > Page 2" );
						Reporter.log("Success !! User is navigated to Unit 4 > Topic 4 > Page 2");		
									
					}
					else
					{
						System.out.println("Failure !! User is NOT navigated to Unit 4 > Topic 4 > Page 2");
						Reporter.log("Failure !! User is NOT navigated to Unit 4 > Topic 4 > Page 2");
						AssertFailedCount++;
					}
	...
	*/ 
	...
	private void Untimed_AtteptFinalExam() throws Exception
	{
		...
		Reporter.log("Step 3 : Attempting all quizes one by one and verifying whether user is allowed to attemp final exam or not"); 
			...
			System.out.println(" -- Attempting Quiz " + 6  + " and marking as Pass" );
			Reporter.log(" -- Attempting Quiz " + 6  + " and marking as Pass");
			
			SeleniumFunc.ToGoToUrl(Constants.AdminURL);
			Thread.sleep(4000);

			keadmin.StudentSearchAndActivity(username, "Snowmobile", "Quizzes");
			keadmin.Quizzes_PassAQuiz(6);
			Thread.sleep(4000);
	...
	*/ 
	...
	/* Test 5: 
	 * Untimed Course - Final Exam :  Verify all buttons works as expected i.e. 'Previous', 'Begin the Certification Exam' 
	*/ 
	@Test()
	private void Untimed_FinalExam_UI_Buttons() throws Exception
	{
		System.out.println("====" + "\n" +
					"Test 5 : Untimed Course - Final Exam :  Verify all buttons works as expected i.e. 'Previous', 'Begin the Certification Exam' "  + "\n" +
		 			"====");
		Reporter.log("====" + "\n" +
		 			  "Test 5 : Untimed Course - Final Exam :  Verify all buttons works as expected i.e. 'Previous', 'Begin the Certification Exam' "  + "\n" +
				 	  "====");	
		...
		System.out.println("Step 6 : On Final Exam page, clicking 'Take the Certification Exam' button" );
		Reporter.log("Step 6 : On Final Exam page, clicking 'Take the Certification Exam' button");
	...
	*/ 
	...
	private void Untimed_FinalExam_Fail() throws Exception
	{
		...
		System.out.println("Step 4 : On Final Exam page, clicking 'Take the Certification Exam' button and attending exam to get result = Fail" );
		Reporter.log("Step 4 : On Final Exam page, clicking 'Take the Certification Exam' button and attending exam to get result = Fail");
	...
	private void Untimed_FinalExam_Pass() throws Exception
	{
		...
		Reporter.log("Step 4 : Verifying Final Exam result page when result = Pass");
			...
			expectedtext = "Next, get your Temporary Snowmobile Safety Education Certificate";
			actualtext = SeleniumFunc.GetElementText("css", "div[class='span8 offset2'] h2");
